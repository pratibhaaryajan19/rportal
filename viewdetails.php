...
if($_POST['submit'])
{
$sapid=secureData($_REQUEST['txtsap']);

$q=mysqli_query($conn,"select * from socs_publication where sap_id='$sapid'  /*AND year='YEAR(CURDATE())'*/ ");
$c=mysqli_num_rows($q);

$qc=mysqli_query($conn,"select * from socs_conference  where sap_id='$sapid'/*AND YEAR(confdate)='YEAR(CURDATE())'*/");
$cc=mysqli_num_rows($qc);

$qp=mysqli_query($conn,"select * from socs_patentdet where sap_id='$sapid'/*AND YEAR(pubdate)='YEAR(CURDATE())'*/");
$cp=mysqli_num_rows($qp);

$qb=mysqli_query($conn,"select * from socs_bookchap where sap_id='$sapid'/*AND YEAR(pubdate)=YEAR(CURDATE())*/");
...
<input type="text" name="txtsap"   required="required" style="width:300px; height:35px"></div>
<div style="text-align:center; margin-top:20px"><input type="submit" name="submit" style="background-color:#FFCC00; height:30px; width:100px" required></div>
</form>
<?php 
}
?>
