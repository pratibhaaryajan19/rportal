...
$q=mysqli_query($conn,"select title_invention, authors from socs_patentdet where YEAR(pubdate)=YEAR(CURDATE())  AND department in (select department from socs_loginnew where sap_id='$uname')");
$c=mysqli_num_rows($q);
...
<body>
<div>
<div  class="container"> <br>PATENT Report-YEAR[<?php echo date('Y'); ?>]</div><br>
...
while($qdisplay=mysqli_fetch_assoc($q)) {
	$cnt=$cnt+1;
echo "<tr><td>" .$cnt ."</td><td>" . $qdisplay['title_invention']. "</td><td>" . $qdisplay['authors'] . "</td></tr>";
}
}
?>
